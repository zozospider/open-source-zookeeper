...
public class ZooKeeper implements AutoCloseable {
     ...
     * deletes the node.
     * 如果 watch 非空且调用成功 (不会引发异常), 则 watch 将留在具有给定路径的节点上.
     * 成功设置该节点上的数据或删除该节点的操作将触发 watch.
     * <p>
     * A KeeperException with error code KeeperException.NoNode will be thrown
     * if no node with the given path exists.
     * 如果不存在具有给定路径的节点, 则将引发错误代码为 KeeperException.NoNode 的 KeeperException.
     *
