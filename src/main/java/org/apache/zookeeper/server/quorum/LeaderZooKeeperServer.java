 ...
 *
 * 就像标准的 ZooKeeperServer 一样. 我们只是替换了 request processors 请求处理器:
...
public class LeaderZooKeeperServer extends QuorumZooKeeperServer {
     ...
     * PrepRequestProcessor
     * 来自 Learner 的 Requests 请求应直接转到 PrepRequestProcessor
     *
     * @param request
     */
    ...
    public void submitLearnerRequest(Request request) {
         ...
         * 来自 Learner 的请求应该已经在已经执行了某些请求验证的每台服务器上通过了 submitRequest(), 因此我们无需再次执行该操作.
         *
         ...
         * 此外, LearnerHandler 仅应在 Leader 的服务器启动时才开始将请求提交到 Leader 的 pipeline 管道中,
         * 因此我们可以将请求直接提交到 PrepRequestProcessor 中.
         *
         * This is done so that requests from learners won't go through
         * LeaderRequestProcessor which perform local session upgrade.
         * 这样做是为了避免来自 Learners 的请求不会通过执行本地会话升级的 LeaderRequestProcessor.
         */
        // 来自 Learner 的请求转发到 Leader 的事务处理链: PrepRequestProcessor
