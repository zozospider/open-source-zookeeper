...
public class NIOServerCnxn extends ServerCnxn {
     ...
     * asynchronous writes.
     * sendBuffer 将字节缓冲区推送到 outgoing buffer (传出缓冲区) 队列中以进行异步写入.
     */
    // 加入到 outgoingBuffers (传出缓冲区) 队列中以进行异步写入.
    // 后续会被当前类的 doIO() 方法调入, 走写逻辑发送出去
    ...
    void doIO(SelectionKey k) throws InterruptedException {
        try {
            ...
            if (k.isReadable()) {
                ...
                // 读取请求内容 Buffer: len + RequestHeader + Request
    ...
    public void sendResponse(ReplyHeader h, Record r, String tag, String cacheKey, Stat stat, int opCode) {
        try {
            // 序列化需要返回的 ReplyHeader 消息头, Record 消息体: packetLength + header (ReplyHeader) + data (Record)
            // 序列化后的 ByteBuffer[] 缓存字节为列表: packetLength + header (ReplyHeader) + data (Record)
            // 然后加入到 outgoingBuffers (传出缓冲区) 队列中以进行异步写入.
            // 后续会被当前类的 doIO() 方法调入, 走写逻辑发送出去
