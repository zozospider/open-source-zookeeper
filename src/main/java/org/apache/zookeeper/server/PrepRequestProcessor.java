...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    // 不断循环从 submittedRequests 队列取出 Request 进行处理
    @Override
    public void run() {
    ...
    private ChangeRecord getRecordForPath(String path) throws KeeperException.NoNodeException {
        ...
        // 从 outstandingChanges 获取已有节点的修改信息, 如果没有就新建一个 (如果节点在 Zookeeper 中不存在, 不会新建)
    ...
    // 添加修改记录到 outstandingChanges (要修改的节点信息队列) 和 outstandingChangesForPath (key: 修改的节点路径 -> value: 要修改的节点信息)
     ...
     * 该方法将在 ProcessRequestThread 内部调用, 该方法是一个单例, 因此将有单个线程调用此代码.
     *
     * @param type
     * @param zxid
     * @param request
    ...
    private void pRequest2TxnCreate(int type, Request request, Record record, boolean deserialize) throws IOException, KeeperException {
        ...
        // 如果是创建顺序节点, 则修改节点路径为父节点的版本号加 10 位的整形值
        ...
        // 判断父节点是否是临时节点, 如果是临时节点则抛异常 (临时节点下不能创建子节点)
        boolean ephemeralParent = EphemeralType.get(parentRecord.stat.getEphemeralOwner()) == EphemeralType.NORMAL;
        if (ephemeralParent) {
            throw new KeeperException.NoChildrenForEphemeralsException(path);
        }

        // 构造 Request 请求的 Record (CreateTxn) 属性
        ...
        // 添加 parentRecord (父节点修改信息) 和 nodeRecord (本节点修改信息) 这两个修改记录
        // 分别添加修改记录到 outstandingChanges (要修改的节点信息队列) 和 outstandingChangesForPath (key: 修改的节点路径 -> value: 要修改的节点信息)

        // 添加 parentRecord 修改记录
        parentRecord = parentRecord.duplicate(request.getHdr().getZxid());
        parentRecord.childCount++;
    ...
    protected void pRequest(Request request) throws RequestProcessorException {
        ...
        // 非事务日志的 TxnHeader hdr 和 Record txn 为 null
        request.setHdr(null);
        request.setTxn(null);
        ...
        // 处理请求链中的下一个
        // 单机版的 nextProcessor 的实现为 SyncRequestProcessor
        // 集群版的 nextProcessor 的实现为 ProposalRequestProcessor
     ...
     * 此方法检查 acl, 以确保其不为 null 或为空, 它具有有效的 schemes 和 ids, 并扩展依赖于发送请求者的身份验证信息的任何关联的 ids.
     *
