 ...
 * way.
 * 此类维护 tree 数据结构. 它没有任何网络或客户端连接代码, 因此可以以独立方式对其进行测试.
 * <p>
 * The tree maintains two parallel data structures: a hashtable that maps from
 * full paths to DataNodes and a tree of DataNodes. All accesses to a path is
 * through the hashtable. The tree is traversed only when serializing to disk.
 * 该 tree 维护两个并行的数据结构: 从完整路径映射到 DataNode 的 hashtable 和 DataNode 的 tree.
 * 对路径的所有访问都是通过 hashtable 进行的. 仅当序列化到磁盘时才遍历该 tree.
 */
// 保存所有数据和结构
public class DataTree {
     ...
     * 该 map 提供了对 datanodes 的快速查找. tree 是真理的源头, 是所有锁定发生的地方
     */
    // 保存所有 DataNode, 结构为: path -> DataNode
    private final NodeHashMap nodes;

    // 所有对 DataNode 的监听: 具体由 WatchManager 实现
    private IWatchManager dataWatches;

    // 所有对 DataNode 的子节点的监听: 具体由 WatchManager 实现
    ...
    public void serializeNodes(OutputArchive oa) throws IOException {
        // 序列化根节点 (根节点的 key 为空字符串, nodes.put("", root);), 包含子节点 (也就等同于序列化所有 DataNodes)
