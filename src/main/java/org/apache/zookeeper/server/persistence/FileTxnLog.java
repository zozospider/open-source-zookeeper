...
public class FileTxnLog implements TxnLog, Closeable {
    ...
    public synchronized boolean append(TxnHeader hdr, Record txn, TxnDigest digest) throws IOException {
        // 如果 TxnHeader 为 null (非事务请求), 则不做任何处理
        if (hdr == null) {
            return false;
        }
        ...
        // 序列化 TxnHeader, Record, TxnDigest
    ...
    public static class FileTxnIterator implements TxnLog.TxnIterator {
        ...
        void init() throws IOException {
            ...
            // 从保存事务日志文件的目录中获取所有的事务日志文件 (返回列表通过文件名中的 zxid 升序排列)
            List<File> files = Util.sortDataDir(
                    FileTxnLog.getLogFiles(logDir.listFiles(), 0),
                    LOG_FILE_PREFIX,
                    false);
            // 获取大于 zxid 的所有事务日志文件 (包含小于 zxid 的最后一个事务日志文件)
            for (File f : files) {
            ...
            // 跳到下一个事务日志文件 (即最小的 zxid 事务日志文件)
            goToNextLog();
            // 获取下一条事务记录 (即最小的 zxid 事务记录)
            next();
        }
