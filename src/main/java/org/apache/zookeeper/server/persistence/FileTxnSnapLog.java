...
public class FileTxnSnapLog {
     ...
     */
    // 构造方法, 创建 TxnLog 事务日志和 SnapShot 快照
    public FileTxnSnapLog(File dataDir, File snapDir) throws IOException {
        LOG.debug("Opening datadir:{} snapDir:{}", dataDir, snapDir);

        // 通过文件目录和版本号, 确定事务日志文件保存目录和快照文件保存目录
     ...
     * @throws IOException
     */
    // 从 SnapShot 和 TxnLog 中快速恢复数据到内存数据库 (ZKDatabase 的 DataTree) 中
    // 同时将请求添加到缓存事务队列 (用于 Follower 与 Leader 同步时, Leader 能快速获取事务的 zxid, 判断 Follower 的同步状态)
    public long restore(DataTree dt, Map<Long, Integer> sessions, PlayBackListener listener) throws IOException {
        long snapLoadingStartTime = Time.currentElapsedTime();
        // 恢复 SnapShot 文件中的数据到 sessions 和 DataTree 中 (反序列化 SnapShot)
        ...
        RestoreFinalizer finalizer = () -> {
            // 再恢复 TxnLog 文件中的数据到 ZKDatabase 的 sessions 和 DataTree 中 (反序列化 TxnLog)
            // 同时将请求添加到缓存事务队列 (用于 Follower 与 Leader 同步时, Leader 能快速获取事务的 zxid, 判断 Follower 的同步状态)
     ...
     * @throws IOException
     */
    // 恢复 TxnLog 文件中的数据到 ZKDatabase 的 DataTree 中 (反序列化 TxnLog)
    // 同时将请求添加到缓存事务队列 (用于 Follower 与 Leader 同步时, Leader 能快速获取事务的 zxid, 判断 Follower 的同步状态)
    public long fastForwardFromEdits(
            DataTree dt,
            Map<Long, Integer> sessions,
            PlayBackListener listener) throws IOException {
        // 从 SnapShot 恢复的最后一个事务日志开始, 继续恢复 TxnLog 中的事务记录
        // 获取 lastProcessedZxid 之后的事务迭代器
        ...
        try {
            // 循环事务迭代器, 依次执行事务日志中的每一条记录, 以还原内存数据库 (ZKDatabase 的 DataTree)
            while (true) {
                // iterator points to
                ...
                try {
                    // 执行一条事务记录, 还原到 ZKDatabase 的 sessions 和 DataTree 中
