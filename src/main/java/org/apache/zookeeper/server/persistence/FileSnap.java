...
public class FileSnap implements SnapShot {
    ...
    public long deserialize(DataTree dt, Map<Long, Integer> sessions) throws IOException {
        ...
        // we should  give up
        // 从保存快照文件的目录中获取最近 100 个有效的快照文件 (返回列表通过文件名中的 zxid 降序排列)
        ...
        // 循环 100 个快照文件列表, 直到获取有效的那个快照文件 (正常情况第 1 个即为最新的有效快照)
     ...
     * @throws IOException
     */
    // 从保存快照文件的目录中获取最近 n 个有效的快照文件
    protected List<File> findNValidSnapshots(int n) throws IOException {
        // 从保存快照文件的目录中获取所有的快照文件 (返回列表通过文件名中的 zxid 降序排列)
     ...
     * @throws IOException
     */
    // 序列化 FileHeader, sessions, DataTree
